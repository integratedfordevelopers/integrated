...
class QueueMessageTest extends \PHPUnit_Framework_TestCase
{
	...
	{
		$message = new QueueMessage($this->data, function() {}, function() {});
	...
	public function testRelease()
	{
		...
		$message = new QueueMessage($this->data, function() use ($delete) { $delete->callback(); }, function($delay) use ($release) { $release->callback($delay); });
	...
	public function testReleaseWithDelay()
	{
		...
		$message = new QueueMessage($this->data, function() {}, function($delay) use ($release) { $release->callback($delay); });
		$message->release(42);
	}
	...
	public function testDelete()
	{
		...
		$message = new QueueMessage($this->data, function() use ($delete) { $delete->callback(); }, function() use ($release) { $release->callback(); });
